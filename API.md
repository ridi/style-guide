...
마이크로서비스를 운영하다보면 내부 리소스 서버간, 즉 신뢰할 수 있는 서버간의 API 통신이 필요한 상황이 발생한다.
이 경우에는 JWT(JSON Web Tokens)를 이용하여 인가를 수행하되 아래의 조건을 만족해야 한다.

- 발급자(iss)와 수신자(aud)에는 팀 명을, 주제(sub)에는 서비스명을 입력한다.
- RS256(RSA Signature SHA-256) 알고리즘으로 서명한다.
  - iss, sub, aud 조합에 따라 구분되는 대칭키 쌍을 사용한다.
  - 비밀키를 사용하는 팀에서 키의 관리를 책임진다.
- `Authentication`헤더의 `Bearer` 토큰으로 전달한다.
