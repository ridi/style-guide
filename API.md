...
이 경우에는 JWT(JSON Web Tokens)를 이용하여 인가를 수행하되 아래의 조건을 만족해야 한다.
 ...
 - JWT는 RS256(RSA Signature SHA-256) 알고리즘으로 서명한다.
 - 요청자(iss)와 주제(sub) 조합에 따라 구분되는 대칭키 쌍을 사용한다.
 - 비밀키를 사용하는 팀(iss)에서 대칭키 쌍의 관리를 책임진다.
 - JWT는 `Authentication`헤더의 `Bearer` 토큰으로 전달한다.
